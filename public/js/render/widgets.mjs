...
export const range_slider = function(steps, kwargs) {
  let { svg, width, height, orient, ctrs, canvas, parent } = kwargs || {};
  
  if (!svg) {
    const panel = render.canvas(parent);
    svg = panel.svg;
    width = panel.width;
    height = panel.height;
  }
  ...
  .on('drag', function (evt, d) {
    ...
    // render.portfolio([]);
    // render.graph(skills_graph, pads, { ...canvas, ctrs });
  });
...
export const scoring = function(values, kwargs) {
  let { parent } = kwargs || {};
  
  parent.addElems('small')
    .html('How relevant is this data?')
  parent.addElems('img', 'star', values)
    .attr('src', '/imgs/star_outline.svg')
}